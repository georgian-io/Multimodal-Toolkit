...
class AlbertWithTabular(AlbertForSequenceClassification):
    ...
    @add_start_docstrings(ALBERT_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
...
class XLMWithTabular(XLMForSequenceClassification):
    ...
    @add_start_docstrings(XLM_INPUTS_DOCSTRING.format("(batch_size, sequence_length)"))
    def forward(
            self,
            input_ids=None,
            attention_mask=None,
